...
import { PubMeetingsMessage } from "./meeting.dto";
...
export class EzService {
  ...
  // 调用集控接口，下发会议信息。
  async pubMeetings(req: PubMeetingsMessage) {
    const [ method, url ] = staticPath.rpc
    this.mutation({ method, url, options: { data: req } })
  }

  // 调用集控接口，获取房间详情。
  async getRoomInfo(applicense:string ) {
    const [ method, url ] = staticPath.room
    return await this.mutation({ method, url , options: { params: { applicense } }})
  }
}
